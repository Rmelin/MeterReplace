...
def availability_dates(db: Session, user_id: int) -> list[date]:
    rows = (
        ...
        .filter(models.VvsAvailability.user_id == user_id)
        .distinct()
...
def task_edit_context(
    ...
    availability = availability_for_date(db, user_id, appointment.starts_at.date())
    return {
...
def edit_task(
    ...
    context = task_edit_context(db, appointment_id, user.id)
    if not context:
        raise HTTPException(status_code=404, detail="Opgave ikke fundet")
...
def update_task(
    ...
    def handle_error(message: str):
        ...
        return RedirectResponse(f"/vvs/tasks/{appointment_id}/edit", status_code=303)
